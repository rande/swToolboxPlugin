 ...
 */
class sfTestFunctionalForkedTask extends sfTestFunctionalTask
{
  /**
   * @see sfTask
   */
  protected function configure()
  {
    ...
    $this->addOptions(array(
      new sfCommandOption('process', null, sfCommandOption::PARAMETER_OPTIONAL, 'The number of simultanious forked processes', 3),
      new sfCommandOption('output_dir', null, sfCommandOption::PARAMETER_OPTIONAL, 'The path where the log files will be stored (content is deleted each times)', sfConfig::get('sf_test_dir').'/output')
    ));

    $this->aliases = array('test-functional-forked', 'tff');
    $this->namespace = 'test';
    $this->name = 'functional-forked';
    $this->briefDescription = 'Launches functional tests with a sub process (fork)';

    $this->detailedDescription = <<<EOF
The [test:functional|INFO] task launches functional tests for a
given application with fork:

  [./symfony test:functional-forked frontend|INFO]
